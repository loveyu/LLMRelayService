...
export function ConcurrencyRulesPage({ onUnauthorized }: { onUnauthorized: () => void }) {
  ...
  return <div className="space-y-5">
    ...
    <Card><CardHeader><CardTitle>渠道与模型实时并发</CardTitle><CardDescription>仅显示当前存在在途请求的渠道，并在其下方按模型拆分。</CardDescription></CardHeader><CardContent>{data?.channels.length ? <div className="space-y-3">{data.channels.map((channel) => <div key={channel.channel} className="border"><div className="flex items-center justify-between px-3 py-2 font-mono text-xs"><span>{channel.channel}</span><span>当前并发 {channel.activeRequests}</span></div><div className="divide-y border-t">{channel.models.map((model) => <div key={model.model} className="flex items-center justify-between px-5 py-2 font-mono text-xs text-muted-foreground"><span>{model.model}</span><span>{model.activeRequests}</span></div>)}</div></div>)}</div> : <p className="text-sm text-muted-foreground">当前没有受并发规则管理的在途请求。</p>}</CardContent></Card>
  </div>
}
