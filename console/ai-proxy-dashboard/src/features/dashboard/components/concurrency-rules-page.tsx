...
export function ConcurrencyRulesPage({ onUnauthorized }: { onUnauthorized: () => void }) {
  ...
  return <div className="space-y-5">
    ...
    <Card><CardHeader><CardTitle>规则状态</CardTitle><CardDescription>每 3 秒刷新一次当前活跃上游请求数。</CardDescription></CardHeader><CardContent>{data?.rules.length ? <div className="divide-y border"><div className="grid grid-cols-[1fr_7rem_7rem_7rem] gap-3 px-3 py-2 text-xs font-medium text-muted-foreground"><span>规则</span><span>当前并发</span><span>上限</span><span>绑定渠道</span></div>{data.rules.map((rule) => { const current = runtime.get(rule.id); return <div key={rule.id} className="grid grid-cols-[1fr_7rem_7rem_7rem] gap-3 px-3 py-3 font-mono text-xs"><span>{rule.name}</span><span>{current?.activeRequests ?? 0}</span><span>{rule.maxConcurrency}</span><span>{rule.providerCount}</span></div> })}</div> : <p className="text-sm text-muted-foreground">尚未创建并发规则。</p>}</CardContent></Card>
  </div>
}
