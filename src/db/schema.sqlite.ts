...
export const consoleRequests = sqliteTable('console_requests', {
  ...
  initialCompletedAt: integer('initial_completed_at', { mode: 'number' }).notNull().default(0),
  // 首次被自动故障转移吞掉的 429 快照。最终成功响应仍使用 response* 字段。
  initialRateLimitRoutePrefix: text('initial_rate_limit_route_prefix'),
  initialRateLimitTargetUrl: text('initial_rate_limit_target_url'),
  initialRateLimitRequestModel: text('initial_rate_limit_request_model'),
  initialRateLimitForwardedPayload: text('initial_rate_limit_forwarded_payload'),
  initialRateLimitForwardedHeadersJson: text('initial_rate_limit_forwarded_headers_json'),
  initialRateLimitResponseHeadersJson: text('initial_rate_limit_response_headers_json'),
  initialRateLimitResponsePayload: text('initial_rate_limit_response_payload'),
  initialRateLimitResponsePayloadTruncated: integer('initial_rate_limit_response_payload_truncated').notNull().default(0),
